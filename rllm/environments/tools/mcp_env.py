...
def get_process_manager(mcp_cmd, mcp_args, mcp_env, cache_dir, allowed_tools):
    ...
    pid = os.getpid()
    with _PROCESS_LOCK:
        if pid not in _PROCESS_GLOBAL_MANAGER:
...
class MCPConnectionManager:
    ...
    def __init__(
        ...
        # 如果 Bright Data 60秒都没返回，我们就当做失败，让 LLM 重试或换个 Query
        self.INTERNAL_TOOL_TIMEOUT = 60.0 
    ...
    def execute_tool_calls(self, tool_calls: list[dict[str, Any]]) -> dict[str, str]:
        ...
        if not self.running or not self.worker_thread.is_alive():
            logger.warning("MCP Worker thread is dead. Attempting restart...")
            self.running = False
            self.start()
            ...
            while self.running:
                try:
                    ...
                    if cmd == "init":
                        ...
                        # 并发执行：将任务抛给 asyncio handle，不阻塞循环获取下一个请求
                        asyncio.create_task(self._handle_execution(data, resp_q))
                
                except Exception as e:
                    logger.error(f"Error in MCP loop: {e}")
    ...
    async def _handle_execution(self, tool_calls, resp_q):
        ...
        try:
            ...
            if resp_q: resp_q.put(("error", "Timeout waiting for tool response (60s limit)"))
        except Exception as e:
