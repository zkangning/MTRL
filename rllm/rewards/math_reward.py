...
from typing import Any, List, Union # 新增导入
from rllm.globals import THOUGHT_DELIMITER_END
from rllm.rewards.math_utils.utils import extract_answer, grade_answer_mathd, grade_answer_sympy
from rllm.rewards.reward_types import RewardConfig, RewardOutput, RewardType
...
class RewardMathFn:
    ...
    def __call__(self, task_info: dict, action: str) -> RewardOutput:
        ...
        # Process the ground truth(s)
        ground_truths = task_info.get("ground_truth") or task_info.get("response") or task_info.get("answer")
        if ground_truths is None:
            return RewardOutput(reward=self.config.unk_error_reward, is_correct=False)
        ...
        if isinstance(ground_truths, (str, float, int)):
            ...
            # 如果解包失败导致这里依然是个 dict 或其他类型，返回错误
            # print(f"DEBUG: Unexpected ground_truth type: {type(ground_truths)}")
            return RewardOutput(reward=self.config.unk_error_reward, is_correct=False)
...
def rllm_reward_fn_math(data_source: str, llm_solution: str, ground_truth: Union[str, List[str], Any], extra_info=None, **kwargs):
    """Evaluates mathematical solutions against ground truth answers.
    
    Compatible with both standard formats (str/list) and complex multi-task formats (dict).
    """
    if extra_info is None:
        extra_info = {}
    
    # --- [兼容性修改] 解包 ground_truth ---
    real_ground_truth = ground_truth

    # 如果 ground_truth 是字典（来自多任务 Dataset 构造）
    if isinstance(ground_truth, dict):
        # 1. 尝试提取真正的答案字段
        # 你的构造函数中使用的是 "response"，但也兼容其他常见字段
        if "response" in ground_truth:
            real_ground_truth = ground_truth["response"]
        elif "answer" in ground_truth:
            real_ground_truth = ground_truth["answer"]
        elif "solution" in ground_truth:
            real_ground_truth = ground_truth["solution"]
        
        # 2. 提取并合并嵌套的 extra_info
        # 这对于获取 question/prompt 信息很有用
        if "extra_info" in ground_truth:
            nested_info = ground_truth["extra_info"]
            if isinstance(nested_info, dict):
                extra_info.update(nested_info)
    # ------------------------------------

    reward_config = RewardConfig()
    reward_fn = RewardMathFn(reward_config)

    # 尝试从 extra_info 中恢复原始问题（用于日志或调试）
    problem_text = extra_info.get("prompt", extra_info.get("question", extra_info.get("problem", "")))

    task_info = {
        "problem": problem_text,
        "problem_type": RewardType.MATH,
        "data_source": data_source,
        "ground_truth": real_ground_truth, # 这里传入解包后的纯净答案
        **extra_info # 展开 extra_info 供使用
    }
...
if __name__ == "__main__":
    ...
    # --- 测试用例 2: 新的多任务 Dict 格式 ---
    print("Test Case 2: Multi-task Dict Format")
    ...
    result = rllm_reward_fn_math(
        data_source="math",
        llm_solution="<think>Simple addition</think> The answer is \\boxed{5}",
        ground_truth=complex_ground_truth
    )
